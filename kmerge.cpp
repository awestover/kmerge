...
on the i-th level of the tree, counting up from the bottom you can have 2^i processors for the 2-merge
The 2-merge splits into odd / even steps for as many processors as it has
and the final swaps are also done in parallel
*/
...
// i.e. enforce_order(A, offset+step, step), enforce_order(A, offset+2*step, step), 
...
void neighbor_swaps(int* A, int* B, uint n, uint step, uint offset, uint p) {
    ...
    uint first_b = offset;
    if (A[last_a] > B[first_b])
        std::swap(A[last_a], B[first_b]);
}
...
void serial_merge_pair(int* A, int* B, uint n, uint step, uint offset) {
    int* C = (int*)malloc(2 * n * sizeof(int)); // Allocates memory for 5 integers
    uint a = 0; uint b = 0;
    while (a < n && b < n) {
...
void parallel_merge_pair(int* A, int* B, uint n, uint step, uint offset, uint p) {
...
void kmerge_pll_tree(int** arrays, uint array_size, uint num_arrays, uint first_array, uint p) {
    if (num_arrays > 2) {
        // TODO : have some other threshold 
        if (p > 1) {
            std::thread left_thread([&]() {
                kmerge_pll_tree(arrays, array_size, num_arrays >> 1, first_array, p >> 1);
                });
            std::thread right_thread([&]() {
                kmerge_pll_tree(arrays, array_size, num_arrays >> 1, first_array + num_arrays / 2, p >> 1);
                });
            left_thread.join();
            right_thread.join();
        }
            ...
            kmerge_pll_tree(arrays, array_size, num_arrays >> 1, first_array + num_arrays / 2, 1);
        }
    }
    else {
        parallel_merge_pair(arrays[first_array], arrays[first_array + 1], array_size, 1, 0, p);
    }
}
